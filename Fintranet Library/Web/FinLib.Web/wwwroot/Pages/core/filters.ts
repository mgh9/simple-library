type ISanitize = (value: string) => string

altairApp.filter('ellipsed10chars', function ($filter: ng.IFilterService, $sce: ng.ISCEService, $sanitize: ISanitize) {
    return function (value, size) {
        //return `<span class="ellipsed-10-chars">${_.escape(value)}</span>`;
        var result = `<span class="ellipsed-10-chars" title="${_.escape(value)}">${_.escape(value)}</span>`;
        return $sce.trustAsHtml(result);
    };
});

altairApp.filter('ellipsed20chars', function ($filter: ng.IFilterService, $sce: ng.ISCEService, $sanitize: ISanitize) {
    return function (value, size) {
        var result = `<span class="ellipsed-20-chars" title="${_.escape(value)}">${_.escape(value)}</span>`;
        return $sce.trustAsHtml(result);
    };
});

altairApp.filter('ellipsed30chars', function ($filter: ng.IFilterService, $sce: ng.ISCEService, $sanitize: ISanitize) {
    return function (value, size) {
        var result = `<span class="ellipsed-30-chars" title="${_.escape(value)}">${_.escape(value)}</span>`;
        return $sce.trustAsHtml(result);
    };
});

altairApp.filter('ellipsed40chars', function ($filter: ng.IFilterService, $sce: ng.ISCEService, $sanitize: ISanitize) {
    return function (value, size) {
        var result = `<span class="ellipsed-40-chars" title="${_.escape(value)}">${_.escape(value)}</span>`;
        return $sce.trustAsHtml(result);
    };
});

altairApp.filter('nullToNoTitleString', function ($sanitize: ISanitize) {
...
altairApp.filter('gridFilter', function ($filter, $sce: ng.ISCEService, $sanitize: ISanitize) {
    return function (value, filterName: string, row) {
        if (!filterName) {
        ...
        if (isEnumString) {
            var enumName = filterName.split(',')[1],
                enumNames: ITitleValue<number>[] = window[`${enumName}TitleValues`];
            ...
            switch (filterName) {
                ...
                case 'persianDateTime':
                    value = value || '';
                    result = `<span class="ltr uk-display-inline-block md-color-grey-500">${$filter('persianDate')(value, 'yyyy/MM/dd HH:mm')}</span>`;
                    result = $sce.trustAsHtml(result);
                    break;

                case 'persianDateTimeSeconds':
                    value = value || '';
                    result = `<span class="ltr uk-display-inline-block md-color-grey-500">${$filter('persianDate')(value, 'yyyy/MM/dd HH:mm:ss')}</span>`;
                    result = $sce.trustAsHtml(result);
                    break;
...
altairApp.filter('toActiveDeactive', function ($filter: ng.IFilterService, $sce: ng.ISCEService) {
    return function (value) {
        var result = '';
...
altairApp.filter('toYesNo', function ($filter: ng.IFilterService, $sce: ng.ISCEService) {
    return function (value) {
        var result = '';
...
altairApp.filter('toUserLockStatus', function ($filter: ng.IFilterService, $sce: ng.ISCEService) {
...
altairApp.filter('toAcceptedString', function ($filter: ng.IFilterService, $sce: ng.ISCEService) {
    return function (value) {
        var result = '';
...
altairApp.filter('persianDate', ['$locale', 'PersianDateService', function ($locale, PersianDateService) {
    ...
    function padNumber(num?: any, digits?: any, trim?: any) {
        var neg = '';
        if (num < 0) {
            neg = '-';
            num = -num;
    ...
    function dateGetter(name?: any, size?: any, offset?: any, trim?: any) {
        offset = offset || 0;
    ...
    function dateStrGetter(name?: any, shortForm?: any) {
    ...
    if (!('map' in Array.prototype)) {
        (<any>Array.prototype).map = function (mapper, that /*opt*/) {
